...
func AdminruleSave(c *fiber.Ctx) error {
	...
	resp, err := axios.R().
		...
		SetBody(map[string]interface{}{
			...
			"page":            client.Page,
			"sdata":           client.Sdata,
			"adminrule_id":    client.Adminrule_id,
			"adminrule_name":  client.Adminrule_name,
...
func Agenadminrulehome(c *fiber.Ctx) error {
	...
	log.Println("  Body       :\n", resp)
	log.Println()
	result := resp.Result().(*responseagenrule)
	if result.Status == 200 {
		return c.JSON(fiber.Map{
			"status":   result.Status,
			"message":  result.Message,
			"record":   result.Record,
			"listagen": result.Listagen,
			"time":     time.Since(render_page).String(),
		})
	} else {
		result_error := resp.Error().(*responseerror)
...
func AgenadminruleSave(c *fiber.Ctx) error {
	type payload_agenadminrulesave struct {
		Page                 string `json:"page"`
		Sdata                string `json:"sdata" `
	...
	bearToken := c.Get("Authorization")
	token := strings.Split(bearToken, " ")
	client := new(payload_agenadminrulesave)
	if err := c.BodyParser(client); err != nil {
		c.Status(fiber.StatusBadRequest)
		return c.JSON(fiber.Map{
			"status":  fiber.StatusBadRequest,
	...
	render_page := time.Now()
	axios := resty.New()
	axios.SetTimeout(5 * time.Second)
	resp, err := axios.R().
		SetResult(responsedefault{}).
		SetAuthToken(token[1]).
		SetError(responseerror{}).
